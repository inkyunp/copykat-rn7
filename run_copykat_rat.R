...
# Run the rat (genome="rn7") CopyKAT fork on a Seurat v4/v5 RDS or a plain
# gene x cell counts matrix RDS. Records candidate CNV evidence only; it never
# assigns a final malignant/normal diagnosis (results remain review_required).
#
...
parse_args <- function(args) {
  out <- list(assay = "RNA", sample_id = NULL, cores = 1L, id_type = "auto",
              output = NULL,
              ngene_chr = 5L, low_dr = 0.05, up_dr = 0.1, ks_cut = 0.1,
              win_size = 25L)
  val <- c("input","output","assay","sample-id","cores","id-type",
...
# id.type
anno_ns <- get("full.anno.rn7", asNamespace("copykat"))
id_type <- toupper(opt$id_type)
if (id_type == "AUTO") {
  n_sym <- length(intersect(rownames(counts), anno_ns$mgi_symbol))
...
run_dir <- paste0(opt$output, "_rn7_run")
dir.create(run_dir, showWarnings = FALSE, recursive = TRUE)
old <- setwd(run_dir); on.exit(setwd(old), add = TRUE)

message("[run] running copykat(genome=\"rn7\") ...")
ck <- copykat(rawmat = counts, id.type = id_type, genome = "rn7",
...
result <- list(
  ...
  note = "Candidate CNV evidence from a rat mm10-path fork; not author-validated. Do not treat aneuploid as a diagnosis."
)
